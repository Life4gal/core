...
namespace prometheus::numeric
{
	...
	private:
		...
		PROMETHEUS_COMPILER_NO_UNIQUE_ADDRESS real_engine_type real_engine_;

		[[nodiscard]] constexpr auto engine() -> engine_type& //
			requires(not is_shared_category)
		{
			return real_engine_.engine;
		}
	...
	public:
			...
			requires(is_shared_category)
		{
			...
			{
				return distribution_type{static_cast<T>(to), from}(engine());
			}
		...
		operator()(
			...
			requires(is_shared_category)
		{
			return RANDOM_WORKAROUND_OPERATOR_THIS(Random)get<T>(from, to);
		}
		...
		constexpr auto operator()(
			...
			requires(not is_shared_category)
		{
			return this->template get<T>(from, to);
		}
		...
		constexpr static auto get(const double probability = .5) noexcept(noexcept(boolean_distribution_type{probability}(engine()))) -> bool //
			requires(is_shared_category)
		{
			PROMETHEUS_PLATFORM_ASSUME(probability >= .0 and probability <= 1.);
		...
		constexpr RANDOM_WORKAROUND_OPERATOR_STATIC auto operator()(
			...
			requires(is_shared_category)
		{
			return RANDOM_WORKAROUND_OPERATOR_THIS(Random)get(probability);
		}
		...
		constexpr RANDOM_WORKAROUND_OPERATOR_STATIC auto operator()(
			...
			requires(is_shared_category)
		{
			return RANDOM_WORKAROUND_OPERATOR_THIS(Random)get<Iterator>(from, to);
		}
		...
		constexpr auto operator()(
			...
			requires(not is_shared_category)
		{
			return this->template get<Iterator>(from, to);
		}
		...
		constexpr static auto get(Range& range) noexcept(noexcept(get(std::ranges::begin(range), std::ranges::end(range)))) -> auto //
			requires(is_shared_category)
		{
			return Random::get(std::ranges::begin(range), std::ranges::end(range));
		}
		...
		constexpr RANDOM_WORKAROUND_OPERATOR_STATIC auto operator()(Range& range) RANDOM_WORKAROUND_OPERATOR_CONST
			...
			requires(is_shared_category)
		{
			return RANDOM_WORKAROUND_OPERATOR_THIS(Random)get<Range>(range);
		}
		...
		constexpr auto get(Range& range) noexcept(noexcept(get(std::ranges::begin(range), std::ranges::end(range)))) -> auto //
			requires(not is_shared_category)
		{
			return this->get(std::ranges::begin(range), std::ranges::end(range));
		}
		...
		constexpr auto operator()(Range& range) noexcept(noexcept(std::declval<Random>().template get<Range>(range))) -> auto //
			requires(not is_shared_category)
		{
			return this->template get<Range>(range);
		}
		...
		constexpr static auto get(
			...
			noexcept(noexcept(
				...
				std::ranges::generate_n(
					...
					count,
					[from, to]() noexcept(noexcept(Random::get(from, to))) -> T
					{
		...
		operator()(
			...
			requires(is_shared_category)
		{
			return RANDOM_WORKAROUND_OPERATOR_THIS(Random)get<Container, T>(container, from, to, count);
		}
		...
		constexpr auto get(
			...
			noexcept(noexcept(
				...
				std::ranges::generate_n(
					std::back_inserter(container),
					count,
					[from, to]() noexcept(noexcept(std::declval<Random>().get(from, to))) -> T
					{
						return Random::get(from, to);
					}) //
#endif
			...
			requires(not is_shared_category)
		{
			std::ranges::generate_n(
				std::back_inserter(container),
				count,
				[from, to, this]() noexcept(noexcept(this->get(from, to))) -> T
				{
		...
		constexpr auto operator()(
			...
			requires(not is_shared_category)
		{
			return this->template get<Container, T>(container, from, to, count);
		}
		...
		constexpr RANDOM_WORKAROUND_OPERATOR_STATIC auto operator()(
			...
			requires(is_shared_category)
		{
			return RANDOM_WORKAROUND_OPERATOR_THIS(Random)get<Container, T>(from, to, count);
		}
		...
		constexpr auto operator()(
			...
			requires(not is_shared_category)
		{
			return this->template get<Container, T>(from, to, count);
		}

		template<typename Distribution, typename... Args>
		constexpr static auto get(Args&&... args)
			noexcept(noexcept(Distribution{std::forward<Args>(args)...}(engine()))) -> Distribution::result_type //
			requires(is_shared_category)
		{
			return Distribution{std::forward<Args>(args)...}(engine());
		}
		...
		constexpr auto get(Args&&... args)
			noexcept(noexcept(Distribution{std::forward<Args>(args)...}(std::declval<Random>().engine()))) -> Distribution::result_type
			//
			requires(not is_shared_category)
		{
			return Distribution{std::forward<Args>(args)...}(engine());
		...
		constexpr auto operator()(Args&&... args)
			...
			requires(not is_shared_category)
		{
			return this->template get<Distribution, Args...>(std::forward<Args>(args)...);
		}
	};
	...
	//               ^
	// note: candidate function [with Category = prometheus::numeric::RandomStateCategory::PRIVATE, RandomEngine = prometheus::numeric::RandomEngine<RandomEngineCategory::X_R_S_R, RandomEngineTag::STAR_STAR, RandomEngineBit::BITS_128>, IntegerDistribution = prometheus::numeric::default_int_distribution, FloatingPointDistribution = prometheus::numeric::default_floating_point_distribution, BooleanDistribution = std::bernoulli_distribution]
	// constexpr explicit Random() noexcept(std::is_nothrow_default_constructible_v<engine_type>) //
	//                              ^
	// note: candidate function [with Category = prometheus::numeric::RandomStateCategory::PRIVATE, RandomEngine = prometheus::numeric::RandomEngine<RandomEngineCategory::X_R_S_R, RandomEngineTag::STAR_STAR, RandomEngineBit::BITS_128>, IntegerDistribution = prometheus::numeric::default_int_distribution, FloatingPointDistribution = prometheus::numeric::default_floating_point_distribution, BooleanDistribution = std::bernoulli_distribution]
	// constexpr explicit Random() noexcept //
