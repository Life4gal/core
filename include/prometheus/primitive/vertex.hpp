...
namespace prometheus::primitive
{
	...
	struct basic_vertex final
	{
		...
		using position_value_type = position_type::value_type;
		using uv_value_type = uv_type::value_type;
		using color_value_type = color_type::value_type;
...
#endif
			tuple_element<Index, prometheus::primitive::basic_vertex<PositionType, UvType, ColorType>> // NOLINT(cert-dcl58-cpp)
	{
		using type = prometheus::primitive::basic_vertex<PositionType, UvType, ColorType>::template element_type<Index>;
	};

	template<typename PositionType, typename UvType, typename ColorType>
	struct tuple_size<prometheus::primitive::basic_vertex<PositionType, UvType, ColorType>> // NOLINT(cert-dcl58-cpp)
			: std::integral_constant<std::size_t, 2 + std::is_same_v<UvType, void> ? 0 : 1> {};

	template<typename PositionType, typename UvType, typename ColorType>
	struct formatter<prometheus::primitive::basic_vertex<PositionType, UvType, ColorType>> // NOLINT(cert-dcl58-cpp)
	{
		...
		auto format(
			const prometheus::primitive::basic_vertex<PositionType, UvType, ColorType>& vertex,
			FormatContext& context
		) const noexcept -> auto
		{
			using vertex_type = prometheus::primitive::basic_vertex<PositionType, UvType, ColorType>;
