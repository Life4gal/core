...
#endif
			tuple_element<Index, prometheus::primitive::basic_color> // NOLINT(cert-dcl58-cpp)
	{
		using type = prometheus::primitive::basic_color::value_type;
	};

	template<>
	struct tuple_size<prometheus::primitive::basic_color> // NOLINT(cert-dcl58-cpp)
			: std::integral_constant<std::size_t, 4> {};

	template<>
	struct formatter<prometheus::primitive::basic_color> // NOLINT(cert-dcl58-cpp)
	{
		...
		auto format(const prometheus::primitive::basic_color& color, FormatContext& context) const noexcept -> auto
		{
			return std::format_to(
				context.out(),
				"#{:x}",
				color.to<prometheus::primitive::ColorFormat::A_R_G_B>()
			);
		}
	};
};
