...
namespace prometheus
{
	namespace primitive
	{
		...
		struct [[nodiscard]] PROMETHEUS_COMPILER_EMPTY_BASE basic_rect<2, PointValueType, ExtentValueType> final : meta::dimension<basic_rect<2, PointValueType, ExtentValueType>>
		{
			...
			// warning: missing initializer for member ‘prometheus::primitive::basic_rect<2, float>::<anonymous>’ [-Wmissing-field-initializers]
			...
			{
				// PROMETHEUS_PLATFORM_ASSUME(not empty() and valid());
			...
			{
				// PROMETHEUS_PLATFORM_ASSUME(not empty() and valid());
				// PROMETHEUS_PLATFORM_ASSUME(not rect.empty() and rect.valid());
				// PROMETHEUS_PLATFORM_ASSUME(width() > rect.width() and height() > rect.height());
		...
		template<typename PointValueType, typename ExtentValueType>
			requires (std::is_arithmetic_v<PointValueType> and std::is_arithmetic_v<ExtentValueType>)
		struct [[nodiscard]] PROMETHEUS_COMPILER_EMPTY_BASE basic_rect<3, PointValueType, ExtentValueType> final : meta::dimension<basic_rect<3, PointValueType, ExtentValueType>>
		{
			using point_value_type = PointValueType;
			using extent_value_type = ExtentValueType;
			...
			// warning: missing initializer for member ‘prometheus::primitive::basic_rect<3, float>::<anonymous>’ [-Wmissing-field-initializers]
			// {.point = point, .extent = extent};
			//                                                   ^
			// No initialization value specified for base class `meta::dimension`
			constexpr basic_rect() noexcept
			...
			[[nodiscard]] constexpr auto includes(const basic_rect& rect) const noexcept -> bool
			{
				// PROMETHEUS_PLATFORM_ASSUME(not empty() and valid());
				// PROMETHEUS_PLATFORM_ASSUME(not rect.empty() and rect.valid());
				// PROMETHEUS_PLATFORM_ASSUME(width() > rect.width() and height() > rect.height() and depth() > rect.depth());
...
#endif
			tuple_element<Index, prometheus::primitive::basic_rect<N, PointValueType, ExtentValueType>> // NOLINT(cert-dcl58-cpp)
	{
		using type = prometheus::primitive::basic_rect<N, PointValueType, ExtentValueType>::template element_type<Index>;
	};

	template<std::size_t N, typename PointValueType, typename ExtentValueType>
	struct tuple_size<prometheus::primitive::basic_rect<N, PointValueType, ExtentValueType>> // NOLINT(cert-dcl58-cpp)
			: std::integral_constant<std::size_t, 2> {};

	template<std::size_t N, typename PointValueType, typename ExtentValueType>
	struct formatter<prometheus::primitive::basic_rect<N, PointValueType, ExtentValueType>> // NOLINT(cert-dcl58-cpp)
	{
		...
		auto format(const prometheus::primitive::basic_rect<N, PointValueType, ExtentValueType>& rect, FormatContext& context) const noexcept -> auto
		{
