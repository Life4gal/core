...
namespace prometheus
{
	namespace primitive
	{
		...
		struct [[nodiscard]] PROMETHEUS_COMPILER_EMPTY_BASE basic_circle final : meta::dimension<basic_circle<N, PointValueType, RadiusValueType>>
		{
			...
			// warning: missing initializer for member ‘prometheus::primitive::basic_circle<2, float>::<anonymous>’ [-Wmissing-field-initializers]
		...
		{
			const auto radius = [r = circle.radius] noexcept -> RadiusValueType
			{
				if constexpr (std::is_floating_point_v<RadiusValueType>)
				{
					return r * std::numbers::sqrt2_v<RadiusValueType>;
				}
				else if constexpr (sizeof(RadiusValueType) == sizeof(float))
				{
					return static_cast<RadiusValueType>(static_cast<float>(r) * std::numbers::sqrt2_v<float>);
				}
				else
				{
					return static_cast<RadiusValueType>(static_cast<double>(r) * std::numbers::sqrt2_v<double>);
				}
			}();
...
#endif
			tuple_element<Index, prometheus::primitive::basic_circle<N, PointValueType, RadiusType>> // NOLINT(cert-dcl58-cpp)
	{
		using type = prometheus::primitive::basic_circle<N, PointValueType, RadiusType>::template element_type<Index>;
	};

	template<std::size_t N, typename PointValueType, typename RadiusType>
	struct tuple_size<prometheus::primitive::basic_circle<N, PointValueType, RadiusType>> // NOLINT(cert-dcl58-cpp)
			: std::integral_constant<std::size_t, 2> {};

	template<std::size_t N, typename PointValueType, typename RadiusType>
	struct formatter<prometheus::primitive::basic_circle<N, PointValueType, RadiusType>> // NOLINT(cert-dcl58-cpp)
	{
		...
		auto format(const prometheus::primitive::basic_circle<N, PointValueType, RadiusType>& circle, FormatContext& context) const noexcept -> auto
		{
