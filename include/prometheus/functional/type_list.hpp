...
namespace prometheus::functional
{
	namespace type_list_detail
	{
		...
		{
			using type = nth_type_impl<Index - 1, Ts...>::type;
		};
		...
		struct list
		{
			...
			using nth_type = nth_type_impl<Index, Ts...>::type;
		...
		public:
			...
			[[nodiscard]] constexpr static auto pop_back() noexcept -> auto
			{
				...
				else if constexpr (N == types_size)
				{
					return list<>{};
				}
				else
			...
			template<template<typename> typename Prediction>
			[[nodiscard]] constexpr static auto sub_list() noexcept -> sub_list_impl<Prediction, list, list<>>::type
			{
				return {};
			}
