	...
	namespace functor_detail
	{
		...
		struct unary
		{
				...
				requires(std::is_invocable_r_v<bool, decltype(Default), Args> and ...)
			{
				...
				if constexpr (Category == UnaryFoldCategory::ALL)
				{
					return (Default(args) and ...);
				}
				else if constexpr (Category == UnaryFoldCategory::ANY)
				{
					return (Default(args) or ...);
				}
				else if constexpr (Category == UnaryFoldCategory::NONE)
				{
					return not(Default(args) or ...);
				}
				else
				{
		...
		struct binary
		{
				...
				) -> const auto&
			{
				...
				{
					return Default(lhs, rhs) ? lhs : rhs;
				}
				else
				{
					return
				...
				) -> const auto&
			{
				if constexpr (sizeof...(reset) == 0)
				{
					return function(lhs, rhs) ? lhs : rhs;
				}
				else
				{
					return
