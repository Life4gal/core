...
namespace prometheus::functional
{
	...
	public:
		...
		template<typename T>
			requires(type_list.template any<T>())
		constexpr auto destroy() noexcept(std::is_nothrow_destructible_v<T>) -> void
		{
		...
		template<typename T>
			requires(type_list.template any<T>())
		[[nodiscard]] constexpr auto load() noexcept -> T&
		{
		...
		template<typename T>
			requires(type_list.template any<T>())
		[[nodiscard]] constexpr auto load() const noexcept -> const T&
		{
		...
		[[nodiscard]] constexpr explicit operator T() const noexcept(std::is_nothrow_copy_constructible_v<T>)
		{
			return load<T>();
		}
		...
		[[nodiscard]] constexpr auto equal(const AlignedUnion& other) const
			noexcept(noexcept(std::declval<const T&>() == std::declval<const T&>())) -> bool
		{
			return load<T>() == other.load<T>();
		}
