// This file is part of prometheus.meta
...
namespace prometheus::meta
{
	...
	namespace string_detail
	{
		...
		template<typename Container, typename Getter, typename SizeType = Container::size_type>
		...
		template<typename Container, typename Getter, typename Comparator, typename SizeType = Container::size_type, typename ValueType = Container::value_type>
		...
		template<typename Container, typename SizeType = Container::size_type>
		...
		{
			using pointer = pointer_traits<DerivedType>::type;
		...
		private:
				...
				requires(not lazy_is_static<derived_type>())
			{
				...
				{
					return static_cast<lazy_pointer_traits<derived_type>::pointer>(static_cast<const derived_type&>(*this).value);
				}
				else if constexpr (lazy_pointer_traits<derived_type>::category == MetaStringDerivedCategory::MEMBER_FUNCTION)
				{
					return static_cast<lazy_pointer_traits<derived_type>::pointer>(static_cast<const derived_type&>(*this).data());
				}
				else
				{
				...
				requires(lazy_is_static<derived_type>())
			{
				...
				{
					return static_cast<lazy_pointer_traits<derived_type>::pointer>(derived_type::value);
				}
				else if constexpr (lazy_pointer_traits<derived_type>::category == MetaStringDerivedCategory::STATIC_FUNCTION)
				{
					return static_cast<lazy_pointer_traits<derived_type>::pointer>(derived_type::data());
				}
				else
				{
					PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE();
		...
		public:
				...
				requires(not lazy_is_static<derived_type>())
			{
				...
				return std::ranges::all_of(
					...
					[comparator](const auto& pair) noexcept(noexcept(comparator(pair.first, pair.second))) -> auto
					{
						return comparator(pair.first, pair.second);
					},
					[this, &container, getter](const auto index) noexcept(noexcept(getter(container, index))) -> auto
					{
				...
				requires(lazy_is_static<derived_type>())
			{
				...
				return std::ranges::all_of(
					...
					[&container, getter](const auto index) noexcept(noexcept(getter(container, index))) -> auto
					{
						return std::make_pair(std::cref(getter(container, index)), std::cref(rep_value()[index]));
					}
				);
	...
	struct basic_char_array : string_detail::meta_string_base<basic_char_array<T, Cs...>, T, std::size_t>
	{
		...
		[[nodiscard]] constexpr static auto begin() noexcept -> const_pointer
		{
			return std::ranges::begin(value);
		}

		[[nodiscard]] constexpr static auto end() noexcept -> const_pointer
		{
		...
		template<std::size_t N = max_size>
			requires(N <= max_size)
		[[nodiscard]] constexpr static auto as_fixed_string() noexcept -> auto
		{
	...
	[[nodiscard]] constexpr auto to_char_array() noexcept -> auto
	{
		...
		return []<std::size_t... Index>(const std::index_sequence<Index...>) noexcept -> auto
		{
...
namespace std
{
	...
	template<prometheus::meta::basic_char_array_t FixedString, typename String, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<prometheus::meta::basic_string_view<typename FixedString::value_type>, String>
	struct basic_common_reference<FixedString, String, Q1, Q2> // NOLINT(cert-dcl58-cpp)
	{
		using type = prometheus::meta::basic_string_view<typename FixedString::value_type>;
	};

	template<typename String, prometheus::meta::basic_char_array_t FixedString, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<prometheus::meta::basic_string_view<typename FixedString::value_type>, String>
	...
	// for `std::totally_ordered_with`
	template<prometheus::meta::basic_fixed_string_t FixedString, typename String, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<prometheus::meta::basic_string_view<typename FixedString::value_type>, String>
	struct basic_common_reference<FixedString, String, Q1, Q2> // NOLINT(cert-dcl58-cpp)
	{
		using type = prometheus::meta::basic_string_view<typename FixedString::value_type>;
	...
	template<typename String, prometheus::meta::basic_fixed_string_t FixedString, template<typename> typename Q1, template<typename> typename Q2>
		requires std::is_constructible_v<prometheus::meta::basic_string_view<typename FixedString::value_type>, String>
	struct basic_common_reference<String, FixedString, Q1, Q2> // NOLINT(cert-dcl58-cpp)
	{
		using type = prometheus::meta::basic_string_view<typename FixedString::value_type>;
