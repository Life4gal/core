...
namespace prometheus::meta
{
	...
	template<std::size_t N, typename T>
	using member_type_of_index = member_detail::member_type_of_index<N, T>::type;
	...
	namespace member_detail
	{
		...
		[[nodiscard]] constexpr auto member_index() noexcept -> std::size_t
		{
			...
			{
				const auto f = []<std::size_t I>() noexcept -> std::size_t
				{
				...
				// clang-format off
				PROMETHEUS_COMPILER_DISABLE_CLANG_WARNING(-Wunused-value)
				// clang-format on
		...
		[[nodiscard]] constexpr auto member_of_name(T&& object) noexcept -> decltype(auto)
		{
			...
			else
			{
				...
				if constexpr (sub_ref_index == std::string_view::npos and sub_ptr_index == std::string_view::npos)
				{
					...
					{
						PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE("If you see this message, you've passed in the wrong member name.");
					}
					else
					{
				...
				{
					const auto make_sub_name = []<std::size_t N>(const auto it) noexcept -> auto
					{
...
#include <prometheus/meta/member.visit.inl>
