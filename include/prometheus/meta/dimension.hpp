...
namespace prometheus::meta
{
	...
	namespace dimension_detail
	{
		...
		struct dimension_walker
		{
			...
			PROMETHEUS_COMPILER_DISABLE_WARNING(4244)
#endif
...
namespace std
{
	...
	struct tuple_element<Index, prometheus::meta::dimension_detail::dimension_wrapper<T, N>> // NOLINT(cert-dcl58-cpp)
	{
		using type = T;
	};

	template<typename T, std::size_t N>
	struct tuple_size<prometheus::meta::dimension_detail::dimension_wrapper<T, N>> : std::integral_constant<std::size_t, N> {}; // NOLINT(cert-dcl58-cpp)
}

#include <prometheus/meta/dimension.cache.inl>
