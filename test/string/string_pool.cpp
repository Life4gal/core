...
// string::string_pool
#include <prometheus/string/string.hpp>
// numeric::random
#include <prometheus/numeric/numeric.hpp>
// ut
...
namespace
{
	...
	// #define WORKAROUND_MSVC_C3688 using namespace ut;
	// #else
	...
	PROMETHEUS_COMPILER_NO_DESTROY ut::suite<"string.string_pool"> _ = [] noexcept -> void
	{
		using namespace ut;
		...
		const auto test_pool = []<typename CharType, bool IsNullTerminate>() noexcept -> void
		{
			...
			"copy_and_move"_test = [] noexcept -> void
			{
				using pool_type = StringPool<CharType, IsNullTerminate>;
		...
		"pool"_test = [test_pool] noexcept -> void
		{
		...
		"block_size"_test = [test_block_size] noexcept -> void
		{
