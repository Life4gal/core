...
auto main() noexcept -> int
{
	...
	auto& config = ut::get_config();
	// config.report_level = ut::config_type::ReportLevel::ALL;
	...
	if (call_debugger_if_fail.has_value())
	{
		...
		{
			config.break_point_level |= ut::config_type::BreakPointLevel::FAILURE;
		}
		else
		{
			config.break_point_level &= ~ut::config_type::BreakPointLevel::FAILURE;
	...
	if (call_debugger_if_fatal.has_value())
	{
		...
		{
			config.break_point_level |= ut::config_type::BreakPointLevel::FATAL;
		}
		else
		{
			config.break_point_level &= ~ut::config_type::BreakPointLevel::FATAL;
		...
		option.set())
	{
		...
		{
			config.filter_suite = [](const ut::suite_node_type& node) noexcept -> bool
			{
				std::ignore = node;
				return true;
			};
		...
		{
			config.filter_suite = [suite = *std::move(suite_names)](const ut::suite_node_type& node) noexcept -> bool
			{
		...
		option.set())
	{
		if (const auto wildcard = option.as<std::string_view>();
			wildcard.has_value() and *wildcard == "*")
		{
			config.filter_test = [](const ut::test_node_type& node) noexcept -> bool
			{
				std::ignore = node;
				return true;
			};
		...
		{
			config.filter_test =
					[test = *std::move(test_names)](const ut::test_node_type& node) noexcept -> bool
					{
