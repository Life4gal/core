...
#include <prometheus/functional/functional.hpp>
// ut
...
namespace
{
	PROMETHEUS_COMPILER_NO_DESTROY ut::suite<"functional.function_ref"> _ = [] noexcept -> void
	{
		using namespace ut;
		using namespace functional;

		"functor"_test = [] noexcept -> void
		{
			struct functor
			{
				constexpr auto operator()(const int a, const int b) const noexcept -> int
				{
					return a + b;
				}

				constexpr auto operator()(int& a) const noexcept -> void
				{
					a = 42;
				}
			};
		...
		"function pointer"_test = [] noexcept -> void
		{
			const auto f = +[](const int a, const int b) noexcept -> int
			{
				return a + b;
			};
		...
		"lambda"_test = [] noexcept -> void
		{
			...
			{
				int i = 42;
				const auto f = [&i](const int a, const int b) noexcept -> int
				{
					return i + a + b;
				};
		...
		"member function"_test = [] noexcept -> void
		{
			class Foo
			{
				int _{0};
			...
			const auto a = ref<int, Foo&, int, int>([](Foo& f, const int v1, const int v2) noexcept -> int
			{
				return f.bar(v1, v2);
			});
